function result = fine_solver(t0, y0, deltaT, sigma, sigmaprime, shape)

% 1 - Use ODE45 for operator F() OR

options = odeset('RelTol', 1e-6, 'AbsTol', 1e-6);
% [~, y] = ode45(@(t, y) ode_function(t, y), [t0, t0 + deltaT], y0, options);
[~, y] = ode45(@(t, y) gradL(y0, sigma, sigmaprime, shape), [t0, t0 + deltaT], y0, options);
result = y(end,:);
...
%     y0 = y0 + dt * gradL(y0, sigma, sigmaprime, shape);
%     t0 = t0 + dt;
% end
% result = y0;
end
